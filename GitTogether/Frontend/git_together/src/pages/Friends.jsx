  ...
  data?.map((friend, key) => (
    ...
    >
      <div className="w-16 h-16 rounded-full overflow-hidden border border-gray-300">
        <img src={friend.fromUserId.photoUrl} alt="Profile" className="object-cover w-full h-full" />
      </div>
      <div>
        <h2 className="text-lg font-semibold">{friend.fromUserId.firstName} {friend.fromUserId.lastName}</h2>
        <p className="text-sm">Age: {friend.fromUserId.age}</p>
        <p className="text-sm">Gender: {friend.fromUserId.gender}</p>
      </div>
    </div>
  ))
}
